...
# Matrice de formation (decalage selon x y z)
format = [
        (0, 0, 0) , (0, 0.4 ,0)
]
...
def transfo_seq_format(seq_format,format):
    ...
    for i in range(len(format)):
        true_seq+=[[]]
        for k in range(len(seq_format)):
            true_seq[i]+=[[
                seq_format[k][0]+format[i][0],
                seq_format[k][1]+format[i][1],
                seq_format[k][2]+format[i][2],
                seq_format[k][3],
                ]]
    return true_seq
...
if __name__ == '__main__':
    ...
    true_seq=transfo_seq_format(seq_format,format)
    seq_args={
        URI1: [true_seq[0]],
        URI2: [true_seq[1]],
    }
