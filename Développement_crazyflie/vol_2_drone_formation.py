...
def seq_unaire(Drone,cf,current):
    ...
    if Drone.leader!=1:
      cf.commander.send_position_setpoint(objectif_tmp[0],objectif_tmp[1],objectif_tmp[2], 0)
        ...
        if current//10==0:
            ...
            cf.commander.send_position_setpoint(0.5,0.5,0.2, 0)
...
def run_sequence(scf):
    ...
    while time.time() < end_time:
        current=time.time()-end_time+90
        print(time.time() - end_time)
        seq_unaire(drone,cf,current)
        time.sleep(0.05)
